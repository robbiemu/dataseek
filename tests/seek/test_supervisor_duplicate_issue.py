...
def test_supervisor_prevents_duplicate_consecutive_agent_calls():
    ...
    state = DataSeekState(
        messages=[
            ...
            AIMessage(
                content="Research completed after 3 iterations. Found 3 research findings..."
            ),
            AIMessage(
                content="**Inspection Report**\n# | Source (query) | Assessment..."
            ),  # Fitness report
        ],
        research_findings=[
    ...
    with (
        ...
        assert not (
            len(last_two_decisions) == 2 and last_two_decisions[0] == last_two_decisions[1]
        ), (
